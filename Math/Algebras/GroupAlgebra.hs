...
inv :: (Algebra (Vect Q (Glex (X b))) b, Num b, Ord b, Show b) => Vect Q b -> Either [Vect Q (Glex (X b))] [Vect Q (Glex (X b))]
inv x@(V ts) =
...
maybeInverse :: (Num b, Algebra (Vect Q (Glex (X b))) b, Ord b, Show b) => Vect Q b -> Maybe (Vect Q b)
maybeInverse x@(V ts) =
    let gs = elts $ map fst $ terms x -- all elements in the group generated by the terms
        cs = map (glexvar . X) gs
        x' = V $ map (\(g,c) -> (g, unit c)) ts
